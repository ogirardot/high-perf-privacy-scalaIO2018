...
class PrivacyIntegrationTest extends FlatSpec with Matchers with BeforeAndAfterAll {
  ...
  def testWithEngine(engine: PrivacyEngine): Unit = {
   it should s"handle simple flat datasets with engine: $engine" in {
      val dataset = List(("AAAA", "BBBB", "CCCC", "DDDD"), ("EEEE", "FFFF", "GGGG", "HHHH"))
      val input = spark
      ...
      val strategies = Map(
        Seq(("rdfs:type", "http://schema.org/Person#pseudo")) -> new PrivacyStrategy {
          ...
          override def apply(data: Fix[DataF]): Either[List[PrivacyApplicationFailure], Fix[DataF]] = {
            data match {
              case Fix(GStringF(value)) =>
                val res = new String(Base64.getEncoder.encode(
                  MessageDigest.getInstance("SHA1").digest(value.toString.getBytes("UTF-8")))
                )
      ...
      val schemaFix = Fix[SchemaF](StructF(
          List(
            (
              "first",
              Fix(StringF(
                ColumnMetadata.empty.copy(tags = List(("rdfs:type", "http://schema.org/Person#pseudo")))
              ))
            ),
      ...
      output.first() should be (Row(
        "4lEhcqv4zJ9n/dSetsrPLfcbutM=",
        "BBBB",
        "CCCC",
        "DDDD")
      )
      output.collect()(1) should be(Row(
        "xJw04gFKLaJw7q4H1zByb/3dMZY=",
        "FFFF",
        "GGGG",
        "HHHH"))
    }
    ...
    it should s"handle complex nested structs with engine: $engine" in {
      ...
      val cypher = new PrivacyStrategy {
        ...
        override def apply(data: Fix[DataF]): Either[List[PrivacyApplicationFailure], Fix[DataF]] = {
          data match {
            case Fix(value: GValueF[_]) =>
              val res = new String(Base64.getEncoder.encode(
                MessageDigest.getInstance("SHA1").digest(value.value.toString.getBytes("UTF-8")))
              )
              Right(Fix(GStringF(res)))
        ...
        override def schema[A](input: SchemaF[A]): SchemaF[A] = StringF(input.metadata)
      }
      val strategies = Map(
      ...
      val tableSchema = Fix[SchemaF](StructF(
        List(
          (
            "civility",
            Fix(StructF(
              List(
                (
                  "familyName",
                  Fix(StringF(
                    ColumnMetadata.empty.copy(tags = List(("rdfs:type", "http://schema.org/Person#mask")))
                  ))
                ),
                (
                  "gender",
                  Fix(LongF(
                    ColumnMetadata.empty.copy(tags = List(("rdfs:type", "http://schema.org/Person#interv")))
                  ))
                ),
                (
                  "givenName",
      ...
      val outputSchemaWithPrivacy = Fix[SchemaF](StructF(
          List(
            (
              "civility",
              Fix(StructF(
      ...
      val schemaAsDT = Fix.birecursiveT.cataT(outputSchemaWithPrivacy)(SchemaF.schemaFToDataType)
      output.schema should be(schemaAsDT.asInstanceOf[StructType])

      output.first() should be (Row(
        Row("MARTIN", "NWoZK3kTsExUV00Ywo1G5jlUKKs=", "ZHmSvjodAvqIT7x0Lu6YDXA8D9g="),
